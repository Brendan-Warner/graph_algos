...
{
	return node1->estimated_distance < node2->estimated_distance;
}

void shortest_path_GBFS(grid* node, bool output_file)
{

	if (node->prev != nullptr)
	{
		shortest_path_GBFS(node->prev, output_file);
	}
...
void GBFS_alg(int starth, int startc, int finishh, int finishc, int height, int width, int output, char walls, char waited, bool output_file)
{
	...
	while (next_node.size())
	{
		...
		std::sort(next_node.begin(), next_node.end(), sort_grid_gbfs);
		...
		{
			shortest_path_GBFS(clossest, output_file);
